...
void Device::initSignals()
{
    ...
    if (m_server) {
        connect(m_server, &Server::serverStarted, this, [this](bool success, const QString &deviceName, const QSize &size) {
            ...
            if (success) {
                ...
                // init stream
                m_demuxer->installVideoSocket(m_server->removeVideoSocket());
                m_demuxer->startDecode();
                ...
                // The screen will automatically turn off when the interface is displayed (m_params.display）
    ...
    if (m_demuxer) {
        ...
        connect(m_demuxer, &Demuxer::onStreamStop, this, [this]() {
            disconnectDevice();
...
bool Device::connectDevice()
{
    ...
    QTimer::singleShot(0, this, [this]() {
        ...
        // max size support 480p 720p 1080p Device native resolution
