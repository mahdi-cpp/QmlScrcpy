...
bool Decoder::push(const AVPacket *packet)
{
    ...
    if (!ret) {
        ...
        /*
        // m_conver The conversion of yuv to rgb is done using cpu, which takes up too much cpu, so use opengl to render yuv
        // QImage The copy also consumes a lot of memory, this solution does not consider
        if (!m_conver.isInit()) {
