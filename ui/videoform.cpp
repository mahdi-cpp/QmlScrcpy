...
void VideoForm::switchFullScreen() {
    if (isFullScreen()) {
        // Horizontal full screen fills the full screen, when restored, restore maintains the aspect ratio
...
void VideoForm::wheelEvent(QWheelEvent *event) {
    ...
    if (m_videoWidget->geometry().contains(event->pos())) {
        ...
        QWheelEvent wheelEvent(
                pos, event->globalPosF(), event->pixelDelta(), event->angleDelta(), event->delta(),
                event->orientation(),
...
void VideoForm::resizeEvent(QResizeEvent *event) {
    ...
    QSize curSize = size();
    // Limit the size of VideoForm to not be smaller than keepRatioWidget good size
    if (m_widthHeightRatio > 1.0f) {
        // hor
