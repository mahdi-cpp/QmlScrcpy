...
{
    initialize();
    connect(&qsc::IDeviceManage::getInstance(), &qsc::IDeviceManage::onNewFrame, this,&SquircleRenderer::onFrame);
}
...
void SquircleRenderer::initialize() {
    qDebug() << "SquircleRenderer::initialize()";
...
void SquircleRenderer::setFrameSize(const QSize &frameSize) {
...
const QSize &SquircleRenderer::frameSize() {
    return m_frameSize;
}

void SquircleRenderer::initShader() {
...
void SquircleRenderer::initTextures() {
...
void SquircleRenderer::deInitTextures() {
...
void SquircleRenderer::onFrame(int width, int height, uint8_t *dataY, uint8_t *dataU, uint8_t *dataV, int linesizeY, int linesizeU, int linesizeV) {
//    qDebug() << *dataY;
...
void SquircleRenderer::updateTextures(quint8 *dataY, quint8 *dataU, quint8 *dataV, quint32 linesizeY, quint32 linesizeU, quint32 linesizeV) {
    if (m_textureInited) {
...
void SquircleRenderer::updateTexture(GLuint texture, quint32 textureType, quint8 *pixels, quint32 stride) {
    if (!pixels)
        return;
...
//    //m_shaderProgram.setUniformValue("t", (float) m_t);
//
