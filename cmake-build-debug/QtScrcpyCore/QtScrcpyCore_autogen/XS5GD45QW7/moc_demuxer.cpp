/****************************************************************************
** Meta object code from reading C++ file 'demuxer.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.10.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../../../QtScrcpyCore/src/device/demuxer/demuxer.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'demuxer.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.10.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
QT_WARNING_PUSH
QT_WARNING_DISABLE_DEPRECATED
struct qt_meta_stringdata_Demuxer_t {
    QByteArrayData data[7];
    char stringdata0[63];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_Demuxer_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_Demuxer_t qt_meta_stringdata_Demuxer = {
    {
QT_MOC_LITERAL(0, 0, 7), // "Demuxer"
QT_MOC_LITERAL(1, 8, 12), // "onStreamStop"
QT_MOC_LITERAL(2, 21, 0), // ""
QT_MOC_LITERAL(3, 22, 8), // "getFrame"
QT_MOC_LITERAL(4, 31, 9), // "AVPacket*"
QT_MOC_LITERAL(5, 41, 6), // "packet"
QT_MOC_LITERAL(6, 48, 14) // "getConfigFrame"

    },
    "Demuxer\0onStreamStop\0\0getFrame\0AVPacket*\0"
    "packet\0getConfigFrame"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Demuxer[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       3,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    0,   29,    2, 0x06 /* Public */,
       3,    1,   30,    2, 0x06 /* Public */,
       6,    1,   33,    2, 0x06 /* Public */,

 // signals: parameters
    QMetaType::Void,
    QMetaType::Void, 0x80000000 | 4,    5,
    QMetaType::Void, 0x80000000 | 4,    5,

       0        // eod
};

void Demuxer::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Demuxer *_t = static_cast<Demuxer *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->onStreamStop(); break;
        case 1: _t->getFrame((*reinterpret_cast< AVPacket*(*)>(_a[1]))); break;
        case 2: _t->getConfigFrame((*reinterpret_cast< AVPacket*(*)>(_a[1]))); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        {
            typedef void (Demuxer::*_t)();
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Demuxer::onStreamStop)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (Demuxer::*_t)(AVPacket * );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Demuxer::getFrame)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (Demuxer::*_t)(AVPacket * );
            if (*reinterpret_cast<_t *>(_a[1]) == static_cast<_t>(&Demuxer::getConfigFrame)) {
                *result = 2;
                return;
            }
        }
    }
}

QT_INIT_METAOBJECT const QMetaObject Demuxer::staticMetaObject = {
    { &QThread::staticMetaObject, qt_meta_stringdata_Demuxer.data,
      qt_meta_data_Demuxer,  qt_static_metacall, nullptr, nullptr}
};


const QMetaObject *Demuxer::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Demuxer::qt_metacast(const char *_clname)
{
    if (!_clname) return nullptr;
    if (!strcmp(_clname, qt_meta_stringdata_Demuxer.stringdata0))
        return static_cast<void*>(this);
    return QThread::qt_metacast(_clname);
}

int Demuxer::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QThread::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 3)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 3;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 3)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 3;
    }
    return _id;
}

// SIGNAL 0
void Demuxer::onStreamStop()
{
    QMetaObject::activate(this, &staticMetaObject, 0, nullptr);
}

// SIGNAL 1
void Demuxer::getFrame(AVPacket * _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 1, _a);
}

// SIGNAL 2
void Demuxer::getConfigFrame(AVPacket * _t1)
{
    void *_a[] = { nullptr, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 2, _a);
}
QT_WARNING_POP
QT_END_MOC_NAMESPACE
